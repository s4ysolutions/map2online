...
export class CatalogDefault implements Catalog {
  static async getInstanceAsync(storage: CatalogStorage, wording: Wording, map2styles: Map2Styles, catalogId: string): Promise<Catalog> {
    ...
    const notifyFeaturesVisibility = instance.notifyVisisbleFeaturesChanged;

    await Promise.all(featuresIdsFlat.map(id => storage.readFeatureProps(id)
      .then(props => {
        if (props !== null) {
          const feature = new FeatureDefault(storage, map2styles, props, featuresCache, notifyFeaturesVisibility);
    ...
    await Promise.all(routesIdsFlat.map(id => storage.readRouteProps(id)
      .then(props => {
        if (props !== null) {
          const route = new RouteDefault(storage, wording, map2styles, props, routesCache, featuresIds, featuresCache, notifyFeaturesVisibility);
    ...
    await Promise.all(categoriesIdsFlat.map(id => storage.readCategoryProps(id)
      .then(props => {
        if (props !== null) {
          const category = new CategoryDefault(storage, wording, map2styles, props, categoriesCache, featuresIds, featuresCache, routesIds, routesCache, notifyFeaturesVisibility);
  ...
  private readonly categoriesIds: Record<ID, ID[]> = {};

  private readonly routesIds: Record<ID, ID[]> = {};

  private readonly featuresIds: Record<ID, ID[]> = {};

  private readonly wording: Wording;

  private readonly map2styles: Map2Styles;

  private readonly subjectVisibleFeatures = new Subject<Feature[]>();

  private readonly observableVisisbleFeaturesDebounced = this.subjectVisibleFeatures.pipe(debounceTime(DEBOUNCE_DELAY));

  private notifyVisisbleFeaturesChanged: () => void;
  ...
  constructor(
    ...
    this.categories = new CategoriesDefault(
      storage,
      wording,
      map2styles,
      catalogId,
