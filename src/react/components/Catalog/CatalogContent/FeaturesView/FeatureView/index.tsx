...
import RichTextView from '../../../../UIElements/RichTextView';
...
const AccordionPart: React.FunctionComponent<{ feature: { geometry: Geometry, description: RichText } }> =
  ({feature}): React.ReactElement => {
    const hasDescription = !RichText.isEmpty(feature.description);

    return <div className="accordion-part" >
...
const FeatueView: React.FunctionComponent<{ isLast: boolean; feature: Feature; route: Route; index: number }> = ({
  ...
  return <div className={`accordion ${isLast && isOpen ? 'last' : ''}`} >
    ...
    {isOpen ? <AccordionPart feature={feature} /> : null}
  </div >;
};
