...
import {Select as OlSelectInteraction} from 'ol/interaction';
...
const SelectInteraction: React.FunctionComponent<{ map: Map }> = ({map}): null => {
  const source = React.useContext(activeFeaturesContext);

  const selectInteractionRef = React.useRef<OlSelectInteraction | null>(null);

  useEffect(() => {
    if (selectInteractionRef.current) {
      map.removeInteraction(selectInteractionRef.current);
    }
    selectInteractionRef.current = new OlSelectInteraction({condition: click});
