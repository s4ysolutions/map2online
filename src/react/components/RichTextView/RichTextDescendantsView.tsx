...
import {keyForDescendant} from './key';
    ...
    return <React.Fragment>
      {
        descendants.map((descendant, index) =>
          RichTextElement.isElement(descendant)
            ? <ElementView element={descendant} isLast={index === descendants.length - 1} key={keyForDescendant(descendant)} />
            : RichTextElement.isStyledText(descendant)
              ? <StyledTextView styledText={descendant as StyledText} />
              : <p>
